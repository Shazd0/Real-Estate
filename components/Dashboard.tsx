...
import { Wallet, TrendingUp, TrendingDown, Bell, CheckCircle, XCircle } from 'lucide-react';
                  ...
                  {pendingTxs.map(t => (
                      <div key={t.id} className="bg-white p-4 rounded-xl shadow-sm border border-amber-100">
                          <div className="flex justify-between items-start mb-2">
                              ...
                              <span className="font-black text-slate-800">{t.amount.toLocaleString()} SAR</span>
                          </div>
                          <p className="text-sm text-slate-600 mb-1">{t.details}</p>
                          <div className="text-xs text-slate-400 mb-3">By: {t.createdByName}</div>
                          {t.extraAmount ? <div className="text-[10px] text-blue-600 font-bold mb-1">+ {t.extraAmount} Extra</div> : null}
                          {t.discountAmount ? <div className="text-[10px] text-emerald-600 font-bold mb-1">- {t.discountAmount} Disc</div> : null}
